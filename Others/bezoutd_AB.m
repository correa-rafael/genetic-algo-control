function [R,S,T]=bezoutd(A,B,Hs,Hr,P,comp_t,Te)
%function [Rp,Sp]=bezoutd(A,B,Hs,Hr,P)
...
%comp_t ... scalar used for computing the polynomial T of the controller (optional parameter)
%            if equal to 1 (default value) T=R, if equal to 2 T=sum(R)
%Te ... scalar representing the system's sampling period (used for ploting
%                           the sensitivity function) (optional parameter)
%
%Outputs:
%R=[r0 r1 r2 ...] ... vector of coefficients for resulted controller numerator 
%S=[s0 s1 s2 ...] ... vector of coefficients for resulted controller denominator 
%T=R, if comp_t=1
%T=sum(R), if comp_t~=1

if nargin<5
    error('Error: a minimum of 5 parameters should be given as input to this function');
end
...
% adding the fixed parts Hs and Hr to the controller
S=conv(Sp,Hs);
R=conv(Rp,Hr);
...
if nargin==7
    P = bodeoptions;
    P.PhaseVisible = 'off';P.FreqUnits = 'Hz';P.FreqScale = 'linear';
    figure;bode(feedback(1,filt(conv(B,R),conv(A,S),Te)),P);title('Bezout: S_{yp}');
    figure;bode(feedback(filt(R,S,Te),filt(B,A,Te)),P);title('Bezout: S_{up}');
    figure;pzmap(filt(T,1,Te)*feedback(filt(B,conv(A,S),Te),filt(R,1,Te)));title('Bezout: computed H_{cl} (A and B given as input, R, S, and T calculated)');
end